...
async function mostrarResultadoFinal() {
  try {
    // Obtener datos del ganador mediante API
    const response = await api('winner');
    if (!response?.success) {
    ...
    crearModalResultados(players, winner, isDraw);
...
function crearModalResultados(players, winner, isDraw) {
  // Remover modal anterior si existe
  const modalAnterior = document.getElementById('modal-resultado-final');
  if (modalAnterior) {
  ...
  overlay.id = 'modal-resultado-final';
  overlay.className = 'modal-overlay resultado-final-overlay';
  overlay.style.display = 'flex';
  
  // Crear contenido de la modal
  const modalContent = document.createElement('div');
  modalContent.className = 'modal-content resultado-final-content';
  
  // Obtener nombres de jugadores
  const player1Name = players[1]?.player || 'Jugador 1';
  const player2Name = players[2]?.player || 'Jugador 2';
  const score1 = players[1]?.total || 0;
  const score2 = players[2]?.total || 0;
  ...
  if (isDraw) {
    ...
    mensajeGanador = `¡${winner.player} es el ganador!`;
    claseGanador = winner === players[1] ? 'ganador-1' : 'ganador-2';
  }
  
  modalContent.innerHTML = `
    <div class="resultado-final-card">
      <div class="close-modal" onclick="cerrarModalResultado()">&times;</div>
      <div class="resultado-titulo">
        <h2>🦕 Partida Finalizada 🦕</h2>
      </div>
      
      <div class="resultado-ganador ${claseGanador}">
        <h3>${mensajeGanador}</h3>
      </div>
      
      <div class="resultado-puntos">
        <div class="jugador-resultado ${score1 > score2 ? 'mejor-puntuacion' : ''}">
          <div class="nombre-jugador">${player1Name}</div>
          <div class="puntos-jugador">${score1} puntos</div>
          <div class="desglose-puntos">
            ${crearDesglosePuntos(players[1]?.by_recinct || {})}
          </div>
        </div>
        
        <div class="vs-separador">VS</div>
        
        <div class="jugador-resultado ${score2 > score1 ? 'mejor-puntuacion' : ''}">
          <div class="nombre-jugador">${player2Name}</div>
          <div class="puntos-jugador">${score2} puntos</div>
          <div class="desglose-puntos">
            ${crearDesglosePuntos(players[2]?.by_recinct || {})}
          </div>
        </div>
      </div>
      
      <div class="resultado-acciones">
        <button class="btn btn-primary" onclick="location.reload()">Nueva Partida</button>
        <button class="btn btn-secondary" onclick="cerrarModalResultado()">Cerrar</button>
      </div>
    </div>
  `;
...
function crearDesglosePuntos(puntosRecinto) {
  const recintos = [
  ...
  return recintos.map(recinto => {
    const puntos = puntosRecinto[recinto] || 0;
    if (puntos > 0) {
      return `<div class="punto-detalle">${recinto}: <span class="puntos">${puntos}</span></div>`;
    }
    return '';
  }).filter(Boolean).join('');
}
...
function cerrarModalResultado() {
  const modal = document.getElementById('modal-resultado-final');
  if (modal) {
    modal.remove();
  }
